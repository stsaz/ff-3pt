...
EXP const char* fdkaac_decode_errstr(int e);
...
/**
Return 0 on success;  <0 on error. */
EXP int fdkaac_encode_create(fdkaac_encoder **penc, fdkaac_conf *conf);

EXP void fdkaac_encode_free(fdkaac_encoder *enc);

/**
@samples: input/output audio samples;  -1: flush.
Return the number of bytes written;  0 if more data is needed;  <0 on error. */
EXP int fdkaac_encode(fdkaac_encoder *enc, const short *audio, size_t *samples, char *data);
